...
def calculate_musiq_folder():
    parser = argparse.ArgumentParser()
    parser.add_argument('folder', type=str, help='Path to image folder')
    parser.add_argument('--save_name', type=str, default='musiq.txt')
    args = parser.parse_args()

    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')

    # MUSIQ (no-ref IQA, higher is better)
    ...
    with open(args.save_name, 'w') as f:
        f.write(f'MUSIQ: {mean_score}\n')

    print('MUSIQ:', mean_score)


if __name__ == '__main__':
    calculate_musiq_folder()
